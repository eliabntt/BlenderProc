import os
import bpy
from blenderproc.python.loader.utils.IO import readOBJ, readPC2, writePC2, readPC2Frame
import numpy as np
import bmesh
import sys
...
def write_usd(temppath, filepath, filename, export_animation=True, sf=0, ef=-1, frame_step=1):
...
evaluation_mode='VIEWPORT',default_prim_path=f"/body_{filename}",root_prim_path=f"/body_{filename}",material_prim_path=f"/body_{filename}/materials",
generate_cycles_shaders=False,generate_preview_surface=True,generate_mdl=True,convert_uv_to_st=True,convert_orientation=True,
convert_to_cm=True,export_global_forward_selection='Y',export_global_up_selection='Z',export_child_particles=False,
export_as_overs=False,merge_transform_and_shape=False,export_custom_properties=True,add_properties_namespace=False,export_identity_transforms=False,
apply_subdiv=True,author_blender_name=True,vertex_data_as_face_varying=False, frame_step=frame_step, start=sf, end=ef, override_shutter=False,
init_scene_frame_range=True,export_textures=False,relative_texture_paths=False,light_intensity_scale=1,
convert_light_to_nits=True,scale_light_radius=True,convert_world_material=True,fix_skel_root=True, xform_op_mode='SRT')
...
def export_stl_data(filepath, filename, lobs, zrot):
    ...
    bpy.ops.export_mesh.stl(filepath=os.path.join(filepath,filename,filename+".stl"), check_existing=True, use_selection=True, global_scale=1, ascii=False, use_mesh_modifiers=False, batch_mode='OFF', axis_forward='Y', axis_up='Z')
    bpy.ops.object.delete()


def write_npy_data(filepath, filename, arm_ob, ob, info, frame_step=1):
    bpy.context.scene.frame_current = bpy.context.scene.frame_start
